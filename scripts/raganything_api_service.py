...
async def process_document_and_upload_to_lightrag(pdf_path: str, page_id: int, datasheet_id: int) -> dict:
    ...
    try:
        ...
        try:
            ...
            # Use RAGAnything to process the document with extended timeout
            import asyncio
            try:
                processing_result = await asyncio.wait_for(
                    ...
                    timeout=900  # 15 minute timeout for proper processing
                )
                
                # Extract the actual processed markdown content
                if processing_result and hasattr(processing_result, 'get'):
                    # Try different result formats
                    rag_content = (processing_result.get("content") or 
                                 processing_result.get("text") or 
                                 processing_result.get("markdown") or 
                                 processing_result.get("processed_content") or "")
                    
                    images_processed = (processing_result.get("images") or 
                                      processing_result.get("extracted_images") or 
                                      processing_result.get("image_paths") or [])
                    
                    # If we got a string result directly, use it
                    if isinstance(processing_result, str):
                        rag_content = processing_result
                        images_processed = []
                else:
                    # Handle direct string result
                    rag_content = str(processing_result) if processing_result else ""
                    images_processed = []
                    
            except asyncio.TimeoutError:
                logger.warning("RAGAnything processing timed out after 15 minutes, using fallback")
...
async def get_actual_mineru_output(pdf_path: str) -> dict:
    """Get the actual MinerU processed markdown file and images"""
    try:
        ...
        if markdown_file and os.path.exists(markdown_file):
            logger.info(f"Found MinerU markdown file: {markdown_file}")
            
            # Read the actual processed content
            ...
            logger.warning(f"No MinerU markdown file found for {pdf_name}")
            return {"success": False, "error": "No markdown file found"}
            
    except Exception as e:
        logger.error(f"Error getting MinerU output: {e}")
