...
def main():
    ...
    if 'error' in health:
        ...
        print(f"   ✅ LightRAG Connected: {health.get('lightrag_connected', False)}")
    ...
    print("   ⚠️ Uncomment the code below to test actual page processing")
    ...
    #     print(f"   ✅ Success! Processing time: {result.get('processing_time', 'N/A')}s")
    # else:
...
def cron_job_example():
    ...
    print(f"🔄 CRON Job: Processing {len(pages_to_process)} pages")
    ...
    for page_id in pages_to_process:
        ...
        if status.get('status') == 'ingested':
            print(f"   ⏭️ Page {page_id} already ingested, skipping")
            continue
        ...
        if result.get('success'):
            ...
            print(f"   ❌ Page {page_id} failed: {result.get('error', 'Unknown error')}")
