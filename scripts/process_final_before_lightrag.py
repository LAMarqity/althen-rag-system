#!/usr/bin/env python3
"""
Final processing before LightRAG - enhanced processing that improves empty alt text 
and uploads to Supabase storage WITHOUT uploading to LightRAG
"""
import os
import sys
import asyncio
...
async def process_page_final_before_lightrag(page_id: int):
    """Process page by enhancing alt text and uploading to Supabase storage WITHOUT LightRAG"""
    try:
        logger.info(f"Processing page {page_id} - FINAL BEFORE LIGHTRAG...")
...
"""
            ...
            for datasheet in datasheets:
                ...
                try:
                    # Process with RAGAnything (WITHOUT LightRAG upload)
...
---
*Complete content: Web page + {len(datasheets)} datasheet(s) with {len(all_images_uploaded)} images (enhanced alt text) - READY FOR LIGHTRAG*
"""
        
        logger.info(f"Created document with ENHANCED alt text: {len(combined_content)} characters, {len(all_images_uploaded)} images")
        
        # Upload to Supabase storage (NO LightRAG upload)
        doc_url = await upload_processed_document_to_supabase(
            combined_content,
            page_data,
            {
                "processing_method": "final_before_lightrag",
                ...
                "ready_for_lightrag": True
            }
        )
        
        # Mark page and datasheets as processed (completed status)
        page_update_data = {
            "rag_ingested": True,
            "rag_ingested_at": "now()",
        ...
        logger.info("FINAL BEFORE LIGHTRAG processing complete!")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python process_final_before_lightrag.py <page_id>")
        sys.exit(1)
    
    page_id = int(sys.argv[1])
    result = asyncio.run(process_page_final_before_lightrag(page_id))
    
    if result["success"]:
        print(f"""
SUCCESS! FINAL BEFORE LIGHTRAG PROCESSING
...
Ready for LightRAG: {'YES' if result.get('ready_for_lightrag') else 'NO'}
