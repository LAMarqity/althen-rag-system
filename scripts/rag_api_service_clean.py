...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("[STARTUP] Starting RAG API Service...")
    try:
    ...
    # Shutdown
    logger.info("[SHUTDOWN] Shutting down RAG API Service...")
...
class WebhookTriggerRequest(BaseModel):
    action: str = Field(default="process_unprocessed", description="Action: process_unprocessed, process_page, process_all")
    page_id: Optional[int] = Field(None, description="Specific page ID for process_page action")
    max_pages: int = Field(default=5, description="Maximum pages to process")
    max_datasheets: int = Field(default=10, description="Maximum datasheets per page")
    fast_mode: bool = Field(default=True, description="Use fast processing mode")
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "webhook": "POST /webhook/trigger - Webhook endpoint for n8n integration",
...
@app.post("/webhook/trigger", response_model=WebhookTriggerResponse)
async def webhook_trigger(
    ...
    """
    Webhook endpoint for n8n integration to trigger page and datasheet processing
    
    Actions:
    - process_unprocessed: Process unprocessed pages with their datasheets
    - process_page: Process a specific page by ID
    - process_all: Process all pages (use with caution)
    """
    start_time = datetime.now()
    task_id = f"webhook_{int(start_time.timestamp())}"
    ...
    try:
        logger.info(f"[WEBHOOK] Triggered action: {request.action} (task_id: {task_id})")
        
        results = {"processed": 0, "successful": 0, "failed": 0, "pages": []}
        
        if request.action == "process_page" and request.page_id:
            # Process specific page
            logger.info(f"[WEBHOOK] Processing specific page: {request.page_id}")
            ...
            if "error" not in result:
                ...
                results["pages"].append({"page_id": request.page_id, "status": "success"})
            else:
                results["processed"] = 1
                results["failed"] = 1
                results["pages"].append({"page_id": request.page_id, "status": "failed", "error": result["error"]})
                
        elif request.action in ["process_unprocessed", "process_all"]:
            # Get pages to process
            query = rag_client.supabase.table("new_pages_index").select("*")
            
            if request.action == "process_unprocessed":
                query = query.eq("ingested", False)
            
            pages_response = query.limit(request.max_pages).execute()
            
            # Process each page
            for page in pages_response.data:
                try:
                    logger.info(f"[WEBHOOK] Processing page {page['id']}: {page['url']}")
                    ...
                    if "error" not in result:
                        results["successful"] += 1
                        results["pages"].append({"page_id": page['id'], "status": "success"})
                    else:
                        results["failed"] += 1
                        results["pages"].append({"page_id": page['id'], "status": "failed", "error": result["error"]})
                    
                    # Small delay between pages
                    await asyncio.sleep(1)
                    
                except Exception as e:
                    logger.error(f"[ERROR] Failed to process page {page['id']}: {e}")
                    results["failed"] += 1
                    results["pages"].append({"page_id": page['id'], "status": "error", "error": str(e)})
        ...
        return WebhookTriggerResponse(
            ...
            message=f"Processed {results['processed']} pages: {results['successful']} successful, {results['failed']} failed",
            task_id=task_id,
            action=request.action,
            parameters={
                "max_pages": request.max_pages,
                "max_datasheets": request.max_datasheets,
                "fast_mode": request.fast_mode,
        ...
        logger.error(f"[ERROR] Webhook processing failed: {e}")
...
async def process_page(
    ...
    try:
        logger.info(f"[PROCESS] Processing page {request.page_id} (fast_mode={request.fast_mode})")
...
if __name__ == "__main__":
    logger.info(f"[STARTUP] Starting RAG API Service on {API_HOST}:{API_PORT}")
    logger.info(f"[CONFIG] LightRAG Server: {LIGHTRAG_SERVER_URL}")
    logger.info(f"[CONFIG] API Key configured: {'Yes' if API_KEY != 'your-secure-api-key-here' else 'No (using default)'}")
    
    uvicorn.run(
        "rag_api_service_clean:app",
        host=API_HOST,
        port=API_PORT,
        reload=False,  # Set to True for development
        log_level="info",
        timeout_keep_alive=1800  # 30 minutes to exceed MinerU 20-minute processing
    )